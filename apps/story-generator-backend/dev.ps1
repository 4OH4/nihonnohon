...
# Resolve the current Python executable — this picks up the active virtualenv
...
$backend = Start-Process -PassThru powershell -ArgumentList @(
    "-NoExit",
    "-Command",
    "Set-Location '$backendDir'; & '$pythonExe' -m uvicorn story_generator.main:app --port 8000 --reload"
)
