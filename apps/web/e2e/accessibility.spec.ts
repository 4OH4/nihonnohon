...
test.describe('Visual regression snapshots', () => {
  ...
  test('SettingsMenu Spaces toggle off', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Settings' }).click()
    expect(await page.locator('[data-radix-popper-content-wrapper]').screenshot()).toMatchSnapshot('settings-spaces-off.png')
    await page.keyboard.press('Escape')
  })

  test('InfoPanel idle state', async ({ page }) => {
    await page.goto('/read/genki-i-ch6-tanaka-letter')
    expect(await page.getByLabel('Word lookup panel').screenshot()).toMatchSnapshot('infopanel-idle.png')
  })

  test('InfoPanel found state', async ({ page }) => {
    await page.goto('/read/genki-i-ch6-tanaka-letter')
    await page.getByRole('button', { name: '起きます' }).click()
    expect(await page.getByLabel('Word lookup panel').screenshot()).toMatchSnapshot('infopanel-found.png')
  })
  ...
  test('InfoPanel not-found state', async ({ page }) => {
    ...
    expect(await page.getByLabel('Word lookup panel').screenshot()).toMatchSnapshot('infopanel-not-found.png')
  })
})
