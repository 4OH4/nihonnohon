...
- **Ruby on selection:** the currently selected word **always** shows its ruby above it, regardless of the `rubyMode` setting, and reverts when deselected. Implemented by gating the `<rt>` `invisible` class on `!showRuby && !isActive` in `WordToken` — do not regress this to `!showRuby` alone.
- **Ruby mode:** `rubyMode` (preferenceStore, Settings) is `'all' | 'supplement' | 'none'`, defaulting to `'supplement'`. `'supplement'` shows `<rt>` only for words carrying a `supplementEntry` — i.e. words outside the standard Genki vocabulary — via `showRuby = rubyMode === 'all' || (rubyMode === 'supplement' && supplementEntry != null)`. Use `!= null`: the `supplementMap` prop is optional, and an omitted entry must not read as a supplement hit. Hiding always uses the `invisible` class (`visibility: hidden`) — layout space for `<rt>` is **always reserved**; never use `display: none`, so changing mode or revealing ruby never reflows the text.
